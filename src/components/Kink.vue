  ...
  <section class="kink-item">
    ...
    <div class="kink-ratings">
      <div class="kink-rating" v-for="subcategory in category.subcategories" :key="subcategory">
        <div class="subcategory-text" v-if="category.subcategories.length > 1">{{ displaySubcategoryName(subcategory) }}</div>
    ...
    <div class="kink-actions">
      <button @click="edit()" class="icon-button edit-icon">
        <span class="sr-only tooltip">{{ language === 'fr' ? 'Éditer le kink' : 'Edit Kink' }}</span>
        <span class="icon-edit"></span>
      </button>
      <button @click="remove()" class="icon-button remove-icon">
        <span class="sr-only tooltip">{{ language === 'fr' ? 'Supprimer le kink' : 'Remove Kink' }}</span>
    ...
    <div class="comment-action">
      <button @click="addComment()" class="icon-button">
        <span class="sr-only tooltip">
          <template v-if="!kink.comment">{{ language === 'fr' ? 'Ajouter un commentaire' : 'Add Comment' }}</template>
          <template v-else>{{ language === 'fr' ? 'Voir/Éditer/Supprimer le commentaire' : 'View/Edit/Remove Comment' }}</template>
        </span>
...
export default class Kink extends Vue {
  ...
  get displayName(): string {
    return this.language === 'fr' && this.kink.name_fr ? this.kink.name_fr : this.kink.name;
  }

  displaySubcategoryName(subcategory: string): string {
    if (this.language === 'fr' && this.category.subcategories_fr) {
      const index = this.category.subcategories.indexOf(subcategory);
      if (index !== -1 && this.category.subcategories_fr[index]) {
        return this.category.subcategories_fr[index];
      }
    }
    return subcategory;
  }
  ...
  displayComment(): string {
    if (this.language === 'fr' && this.kink.comment_fr) {
      return this.kink.comment_fr;
    }
    return this.kink.comment || '';
  }
  ...
  async addComment(): Promise<void> {
    await showDialog(CommentDialog, { category: this.category, kink: this.kink, language: this.language });
    if (!this.kink.comment?.trim() && !this.kink.comment_fr?.trim()) {
      this.removeComment();
    }
  }
  ...
  removeComment(): void {
    ...
    this.$set(this.kink, "comment_fr", undefined);
  }
}
</script>
