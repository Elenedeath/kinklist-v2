<template>
  <Dialog
    :titleText="language === 'fr' ? 'Éditer les évaluations' : 'Edit ratings'"
    @close="close()"
  >
    <div class="ratings-dialog-content">
      ...
      <p v-else>
        <strong>Attention :</strong> Bien que vous puissiez modifier les évaluations et les couleurs pour kinklist,
        la sélection actuelle de couleurs a été faite pour être distinguable par les personnes
        souffrant de daltonisme. Modifier les couleurs utilisées est donc déconseillé.
      </p>
      <div class="ratings-head">
        <h3>{{ language === 'fr' ? 'Évaluations' : 'Ratings' }}</h3>
        <button class="icon-button add-icon" @click="add()">
          <span class="sr-only tooltip">{{ language === 'fr' ? 'Ajouter une évaluation' : 'Add rating' }}</span>
        </button>
      </div>
      <ul class="ratings">
        <li class="rating" v-for="(rating, i) in model" :key="`rating-${i}`">
          <label>
            ...
            <input type="text" v-model="model[i].name" :placeholder="language === 'fr' ? 'Nom' : 'Name'">
          </label>
          <label>
            {{ language === 'fr' ? 'Couleur' : 'Color' }}
            <input type="text" v-model="model[i].color" :placeholder="language === 'fr' ? 'Couleur' : 'Color'">
          </label>
          <button class="icon-button remove-icon remove-rating" @click="remove(i)">
            <span class="sr-only tooltip">{{ language === 'fr' ? 'Supprimer l\'\u00e9valuation' : 'Remove rating' }}</span>
          </button>
        </li>
      </ul>
      <div class="actions">
