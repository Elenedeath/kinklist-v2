  ...
  <section class="kink-category">
    <header>
      ...
      <button class="add-icon add-kink icon-button" @click="addKink()">
        <span class="sr-only tooltip">{{ language === 'fr' ? 'Ajouter un kink' : 'Add Kink' }}</span>
      </button>
      <button class="edit-icon edit-category icon-button" @click="editCategory()">
        <span class="sr-only tooltip">{{ language === 'fr' ? 'Éditer la catégorie' : 'Edit category' }}</span>
      </button>
      <button class="remove-icon remove-category icon-button" @click="removeCategory()">
        <span class="sr-only tooltip">{{ language === 'fr' ? 'Supprimer la catégorie' : 'Remove category' }}</span>
      </button>
    </header>
...
export default class Category extends Vue {
    ...
    get displayName(): string {
      return this.language === 'fr' && this.category.name_fr ? this.category.name_fr : this.category.name;
    }
