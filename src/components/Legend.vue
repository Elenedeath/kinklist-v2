<template>
  <div class="legend">
    <button class="icon-button edit-icon edit-legend" @click="editRatings()">
      <span class="sr-only tooltip">{{ language === 'fr' ? 'Éditer les évaluations' : 'Edit ratings' }}</span>
    </button>
    <dl>
      <div
      ...
      >
        ...
        <dd>{{ displayRatingName(rating) }}</dd>
      </div>
    </dl>
  </div>
</template>
...
export default class Legend extends Vue {
  ...
  displayRatingName(rating: Rating): string {
    return this.language === 'fr' && rating.name_fr ? rating.name_fr : rating.name;
  }

  async editRatings(): Promise<void> {
    const newRatings: false | Rating[] = await showDialog(LegendDialog, { ratings: this.ratings, language: this.language });
    if (newRatings) {
