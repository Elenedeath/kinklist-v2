...
export const generateKinklistImage = (inCategories: InKinkCategory[], ratings: Rating[], username: string, encodeData: boolean, language: 'en' | 'fr' = 'en'): HTMLCanvasElement => {
    ...
    drawLegend(context, ratings, canvasWidth, language);
    drawAllColumns(context, columns, ratings, encodeData, inCategories, language);
    if (encodeData) {
...
const drawAllColumns = (context: CanvasRenderingContext2D, columns: ExKinkCategory[][], ratings: Rating[], includeComments: boolean, inCategories: InKinkCategory[], language: 'en' | 'fr' = 'en'): void => {
...
const drawCategory = (context: CanvasRenderingContext2D, x: number, y: number, category: ExKinkCategory, ratings: Rating[], includeComments: boolean, language: 'en' | 'fr' = 'en', originalCat?: InKinkCategory): void => {
    // Draw title
    context.fillStyle = '#000000';
...
const drawLegend = (context: CanvasRenderingContext2D, ratings: Rating[], canvasWidth: number, language: 'en' | 'fr' = 'en'): void => {
    ...
    for (const rating of ratings) {
        ...
        context.fillStyle = '#000000';
        const displayName = language === 'fr' && rating.name_fr ? rating.name_fr : rating.name;
        context.fillText(displayName, x + 15, 22);
